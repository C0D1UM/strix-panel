...
Scan lifecycle: `queued` → `running` → `completed` | `failed` | `stopped`, with `stopping` after a stop request (the worker sees it on its next poll and sends SIGINT, then SIGTERM after 30 s and SIGKILL after 60 s). A worker restart mid-scan marks the scan `failed`. A `failed` or `stopped` scan can be resumed (`POST /api/v1/scans/:id/resume`, rule in `checkScanResume` in `packages/shared`): it goes back to `queued`, and the worker continues its run with `strix -n --resume <runName>`, or starts over when Strix never created a run. Resume is refused when Strix saved no agent snapshot or the budget cap is used up. Usage, findings and `startedAt` carry over. Sandbox containers: the worker sets `STRIX_RUN_ID=<scanId>` and `STRIX_RUN_TYPE=strix-panel`, which Strix turns into container labels; when a scan ends (any outcome) the worker force-removes that scan's containers, and on startup it sweeps containers of scans that are already finished (`apps/worker/src/sandbox.ts`). Status values live in `packages/shared`.
...
- App shell: signed-in pages are children of the `/` route in `src/router.ts`, rendered inside `src/layouts/AppLayout.vue` (sidebar on desktop, collapsible to icons and remembered; drawer on mobile). To add a page, add a child route and an entry in `sections` in `src/components/AppSidebar.vue` (`adminOnly: true` hides it from regular users; the Admin section holds admin-only pages). The sidebar lists only pages that exist, with no placeholders.
